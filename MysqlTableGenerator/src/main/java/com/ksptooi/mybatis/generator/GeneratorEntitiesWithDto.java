...
public class GeneratorEntitiesWithDto implements Generator{
    ...
    public void generate(MtgGenOptions opt, List<TableField> fields) {
        ...
        if(opt.isGenVo()){
            ...
            out = new File(opt.getOutputPath(), TextConv.pkgToPath(opt.getPkgNameVo()) + "\\"+opt.getVoName() + ".java");
            VelocityWrapper.mergeAndOutput(t,vc,out);
            vc.put("dtoName", opt.getDtoName());
            t = VelocityWrapper.getTemplate("dto.vm");
            out = new File(opt.getOutputPath(), TextConv.pkgToPath(opt.getPkgNameDto()) + "\\"+opt.getDtoName() + ".java");
            VelocityWrapper.mergeAndOutput(t,vc,out);
        }
    }
