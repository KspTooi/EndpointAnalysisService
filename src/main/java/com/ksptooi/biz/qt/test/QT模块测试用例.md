...
- 预期：
  ...
  - 任务记录新增 1 条失败记录
  - 任务不再继续调度
- 实际（人工填写）：❌ 新增任务时系统报错,"在处理您的请求时发生了一个内部错误，该错误已被审计系统记录，错误代码: ERR-INTERNAL-10"
  - 任务状态：
  - 上次执行状态：
  - 任务记录变化：
  - 是否持续调度：
...
- 预期：
  ...
  - 任务记录新增 1 条失败记录
  - 任务不再继续调度
- 实际（人工填写）：❌ 新增任务时系统报错,"在处理您的请求时发生了一个内部错误，该错误已被审计系统记录，错误代码: ERR-INTERNAL-11"
  - 任务状态：
  - 上次执行状态：
  - 任务记录变化：
  - 是否持续调度：
...
- 预期：
  ...
  - 任务记录不新增
  - 任务不再继续调度
- 实际（人工填写）：❌ 新增任务时系统报错,"在处理您的请求时发生了一个内部错误，该错误已被审计系统记录，错误代码: ERR-INTERNAL-12"
  - 任务状态：
  - 上次执行状态：
  - 任务记录变化：
  - 是否持续调度：
...
- 前置配置：
  - 目标任务：填写一个不存在的 Bean 名称（例如 `NotExistBean`）
  - 失败策略：选择"自动暂停"（业务异常时自动变更为"暂停（异常）"）
  - 日志策略：选择"全部"（成功和失败都记录）
  - 任务参数：留空
- 步骤：
...
- 预期：
  ...
  - 任务记录新增 1 条失败记录
  - 任务不再继续调度
- 实际（人工填写）：❌ 不能创建任务,无法在下拉框中选择到这个 Bean 名称
  - 任务状态：
  - 上次执行状态：
  - 任务记录变化：
  - 是否持续调度：
