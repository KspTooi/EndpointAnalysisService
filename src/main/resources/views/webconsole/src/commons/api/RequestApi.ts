...
export default {
    ...
    getRequestList: async (dto: GetRequestListDto): Promise<RestPageableView<GetRequestListVo>> => {
        return await Http.postEntity<RestPageableView<GetRequestListVo>>('/ras/request/getRequestList', dto);
    },

    getRequestDetails: async (id:string): Promise<GetRequestDetailsVo> => {

        const res = await Http.postEntity<Result<GetRequestDetailsVo>>('/ras/request/getRequestDetails', { id:id});
        if(res.code === 0){
            return res.data;
        }else{
