...
<template>
    ...
    <!-- 编辑对话框 -->
    <el-dialog v-model="editDialogVisible" :title="`编辑${node?.type === 0 ? '分组' : '请求'}`" width="400px">
        <el-form ref="editFormRef" :model="editForm" :rules="editRules" label-width="80px">
            <el-form-item :label="`${node?.type === 0 ? '分组' : '请求'}名称`" prop="name">
                <el-input v-model="editForm.name" :placeholder="`请输入${node?.type === 0 ? '分组' : '请求'}名称`" />
            </el-form-item>
        </el-form>
        <template #footer>
    ...
    <!-- 新建子组对话框 -->
    <el-dialog v-model="addSubGroupDialogVisible" :title="`为 [${node?.name}] 新建子组`" width="400px">
        <el-form ref="addSubGroupFormRef" :model="addSubGroupForm" :rules="addSubGroupRules" label-width="80px">
            <el-form-item label="子组名称" prop="name">
                <el-input v-model="addSubGroupForm.name" placeholder="请输入子组名称" />
            </el-form-item>
        </el-form>
        <template #footer>
            <span class="dialog-footer" style="gap: 10px; display: flex; justify-content: right;">
