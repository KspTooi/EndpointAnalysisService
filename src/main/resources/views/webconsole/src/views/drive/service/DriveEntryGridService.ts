...
import { ref, onUnmounted, type Ref, reactive } from "vue";
...
export default {
   ...
   * @param emit
   */
  useEntryList(emit: (event: "on-entries-loaded", data: GetEntryListVo[], total: number) => void) {
    ...
    const listLoad = async () => {
      ...
      try {
        ...
        emit("on-entries-loaded", listData.value, listTotal.value);
        return res;
      } catch (error: any) {
   ...
   */
  useEntrySelection(containerRef: Ref<HTMLElement | null>, itemRefsMap: Ref<Map<string, HTMLElement>>) {
    // 是否正在框选
   ...
   */
  useEntryDrag(entryData: Ref<GetEntryListVo[]>, selectedIds: Ref<Set<string>>, emit: (event: "on-entry-drag", target: GetEntryListVo | null, entries: GetEntryListVo[]) => void) {
    // --- 状态 ---
