 ...
 * 新增DTO
 */
export interface AddQtTaskRcdDto {
  taskId: string; // 任务ID
  taskName: string; // 任务名
  groupName: string; // 分组名
  target: string; // 调用目标
 ...
 * 编辑DTO
 */
export interface EditQtTaskRcdDto {
  id: string; // 调度日志ID
  taskId: string; // 任务ID
  taskName: string; // 任务名
  groupName: string; // 分组名
...
export default {
   ...
   * 新增任务调度日志表
   */
  addQtTaskRcd: async (dto: AddQtTaskRcdDto): Promise<string> => {
    const result = await Http.postEntity<Result<string>>("/qtTaskRcd/addQtTaskRcd", dto);
    if (result.code === 0) {
   ...
   * 编辑任务调度日志表
   */
  editQtTaskRcd: async (dto: EditQtTaskRcdDto): Promise<string> => {
    const result = await Http.postEntity<Result<string>>("/qtTaskRcd/editQtTaskRcd", dto);
    if (result.code === 0) {
      return result.message;
    }
    throw new Error(result.message);
