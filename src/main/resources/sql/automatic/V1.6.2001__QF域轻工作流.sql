...
CREATE TABLE qf_todo(
                        ...
                        `summary` VARCHAR(200) NOT NULL  COMMENT '摘要(如：张三提交的 5000 元报销)' ,
                        `member_type` INT NOT NULL  COMMENT '办理成员类型 0:办理人, 1:候选组' ,
                        `member_id` VARCHAR(32) NOT NULL  COMMENT '办理成员ID (用户ID或用户组标识)' ,
                        `initiator_id` BIGINT NOT NULL  COMMENT '发起人ID' ,
                        `initiator_name` VARCHAR(32) NOT NULL  COMMENT '发起人名' ,
...
CREATE TABLE qf_done(
                        ...
                        `table_name` VARCHAR(200) NOT NULL  COMMENT '物理表名(带入业务表单数据)' ,
                        `data_id` BIGINT NOT NULL  COMMENT '物理表数据主键ID' ,
                        `node_name` VARCHAR(80) NOT NULL  COMMENT '处理的节点名称 (如: 部门经理审批)' ,
                        `summary` VARCHAR(200) NOT NULL  COMMENT '摘要(从代办复制)' ,
                        `member_id` BIGINT NOT NULL  COMMENT '实际办理人ID' ,
                        `member_name` VARCHAR(32) NOT NULL  COMMENT '实际办理人姓名' ,
...
CREATE TABLE qf_biz_form(
                            `id` BIGINT NOT NULL  COMMENT '主键ID' ,
                            `name` VARCHAR(32) NOT NULL  COMMENT '业务名称(如采购申请)' ,
                            `code` VARCHAR(32) NOT NULL  COMMENT '业务编码(如PURCHASE_APPLY, 必须全局唯一)' ,
                            `form_type` INT NOT NULL  COMMENT '表单类型 0:手搓表单 1:动态表单' ,
                            `icon` VARCHAR(80) NOT NULL  COMMENT '图标' ,
                            `table_name` VARCHAR(200) NOT NULL  COMMENT '对应物理表名' ,
                            `route_pc` VARCHAR(200)   COMMENT 'PC端路由名' ,
                            `route_mobile` VARCHAR(200)   COMMENT '移动端路由名' ,
