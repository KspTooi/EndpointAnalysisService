<template>
  <StdListContainer>
    ...
    <StdListAreaQuery>
      <el-form :model="listForm" inline class="flex justify-between">
        <div>
          ...
          <el-form-item label="原始字段名">
            <el-input v-model="listForm.name" placeholder="输入原始字段名" clearable />
          </el-form-item>
          <el-form-item label="原始数据类型">
            <el-input v-model="listForm.kind" placeholder="输入原始数据类型" clearable />
          </el-form-item>
          ...
          <el-form-item label="原始排序">
            <el-input v-model.number="listForm.seq" placeholder="输入原始排序" clearable />
          </el-form-item>
        </div>
        <el-form-item>
    ...
    <StdListAreaAction class="flex gap-2">
      <el-button type="success" @click="openModal('add', null)">新增输出方案原始模型表</el-button>
    </StdListAreaAction>
    ...
    <el-dialog
      ...
      :title="modalMode === 'edit' ? '编辑输出方案原始模型表' : '新增输出方案原始模型表'"
      width="600px"
...
const { modalVisible, modalLoading, modalMode, modalForm, modalRules, openModal, resetModal, submitModal } =
  OutModelOriginService.useOutModelOriginModal(modalFormRef, loadList);
</script>
